...
@section('meta-description', 'Upgrade your Music Town account tier for higher withdrawal limits.')
...
@section('content')
        <section style="max-width:700px;margin:0 auto;">
            <div class="section-heading">
                ...
                <p style="color:var(--muted);margin-top:8px;">
                    Current: <strong style="color:var(--blue-soft);text-transform:uppercase;">{{ $user->tier }}</strong>
                    &middot; Balance: <strong style="color:var(--gold);">₦{{ number_format($user->balance, 2) }}</strong>
                </p>
            </div>
            ...
            @php $currentLevel = (int) substr($user->tier, 4); @endphp

            {{-- Show virtual account details if there's a pending funding for upgrade --}}
            @if ($pendingFunding && $virtualAccount)
                @php
                    $fundTier = null;
                ...
                <div class="payment-card">
                    <div class="payment-card-head">
                        ...
                        <strong style="font-size:1.2rem;">₦{{ number_format($pendingFunding->amount, 2) }}</strong>
                    </div>
                    ...
                    <div class="account-detail">
                        ...
                        <div class="copy-row">
                            ...
                            <button type="button" class="copy-btn" onclick="copyText('{{ $virtualAccount->account_number }}')">Copy</button>
                        </div>
                    </div>
                    <div class="account-detail">
            ...
            <div style="display:grid;gap:16px;margin-top:24px;">
                ...
                <div class="tier-card {{ $user->tier === 'tier0' ? '' : 'disabled' }}">
                    <div class="tier-head">
                        <h3>Tier 0</h3>
                ...
                @foreach ($tiers as $tierKey => $tier)
                    ...
                    @endphp
                    <div class="tier-card {{ ($isUpgrade || $isCurrent) ? '' : 'disabled' }}">
                        <div class="tier-head">
                            <h3>{{ ucfirst($tierKey) }}</h3>
                            @if ($isCurrent)
                                <span class="tier-badge current">Current</span>
                            @endif
                        </div>
                        <p class="tier-desc">{{ $tier['desc'] }}</p>
                        <div class="tier-limit">Max withdrawal: <strong>{{ $tier['max'] }}</strong> &middot; Withdrawals: <strong>{{ $tier['count'] }}</strong></div>
                        <div class="tier-cost">Upgrade: <strong>₦{{ number_format($tier['cost']) }}</strong></div>
                        @if ($isUpgrade)
                            <form method="POST" action="{{ route('profile.upgrade') }}" style="margin-top:16px;">
                                @csrf
                                <input type="hidden" name="tier" value="{{ $tierKey }}">
                                <input type="hidden" name="method" value="bank">
                                <button class="button" type="submit" style="width:100%;">Upgrade Now (₦{{ number_format($tier['cost']) }})</button>
                            </form>
                        @endif
                    </div>
                @endforeach
    ...
    <script>
        ...
        function checkUpgradePayment() {
            ...
            fetch('{{ route('profile.upgrade.check') }}', {
                method: 'POST',
                headers: {
